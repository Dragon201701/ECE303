...
scaleFactor = 1/32768; % reduce the values from the ADC to less than 1
...
% scale ISR inputs for the output of a 16-bit ADC
xStorage = 32767*xStorage;
noise = 32767*noise;
...
for j = N:M
    ...
    r(1) = scaleFactor*noise(j); % interference (correlated noise) 
    x = scaleFactor*xStorage(j); % voice + interference 
...
axis([0 M -1.1 1.1])
subplot(3,1,2)
plot(xStorage/32767) % voice plus noise
