...
%
%  You must load the files contained in the equalizer.mat 
%  PRIOR to running this m-file!
%
%  by Thad Welch, 16 September 1998, June 2004
...
[H_1,f]=freqz(filt1.tf.num,filt1.tf.den,2*1024);
[H_2,f]=freqz(filt2.tf.num,filt2.tf.den,2*1024);
[H_3,f]=freqz(filt3.tf.num,filt3.tf.den,2*1024);
[H_4,f]=freqz(filt4.tf.num,filt4.tf.den,2*1024);
[H_5,f]=freqz(filt5.tf.num,filt5.tf.den,2*1024);
...
% linear phase ... maintained
% figure(7)
% plot(f*FsOver2/pi/1000,unwrap(angle(A(1)*H_1 + A(2)*H_2 + A(3)*H_3 + A(4)*H_4 + A(5)*H_5)),'b')
