...
{                    
	...
	float maxOutput;
	Uint8 workingLedMask = 0;  				// all LEDs off
	...
	if(++DecimationIndex >= DecimationFactor) { // use sample?
		...
		if (maxOutput > 28000) {
			...
			if (LED_1_counter) {
				LED_1_counter--;
				workingLedMask |= LED1_BIT;  // keep LED1 on
			}
		}
		...
		if (maxOutput > 32000) {
			...
			if (LED_2_counter) {
				LED_2_counter--;
				workingLedMask |= LED2_BIT;  // keep LED2 on
			}
		}
		...
		if (maxOutput > 32767) {
			...
			if (LED_3_counter) {
				LED_3_counter--;
				workingLedMask |= LED3_BIT;  // keep LED3 on
			}
		}
