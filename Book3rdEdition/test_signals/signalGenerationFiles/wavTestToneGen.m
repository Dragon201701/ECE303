%  wavTestToneGen.m
%
%  This m-file creates a test tone which is converted to a wav-file and stored 
%  to disk.  This file may then be played using any wav-capable device.  The
%  file can also be transferred to other media (e.g. CD-R or CD-RW).  Once
%  transferred to CD, a CD player can be used as a function generator.
%
...
%
%  "testTone" values MUST be kept within the range -1.0 to 1.0 or clipping will 
...
toneFreq = 1000;    % test tone frequency

Fs = 44100;         % sample frequency in samples/sec (CD players use 44100)
Nbits = 16;         % number of bit used to represent a sample
scaleFactor = 0.99; % ensures that the |testTone| < 1.0

% Calculated terms
testTone = scaleFactor*cos(2*pi*toneFreq*(0:(1/Fs):duration));

%Simulation outputs
wavwrite(testTone, Fs, Nbits, 'C:\MATLAB6p1\work\wavFiles\testTone1000.wav')